...
class Agent(Player):
    ...
    def action(self,  state=None):
        ...
        if list_co_the_lay.__len__() != 0:
            ...
            # check xem có thể úp thẻ hay ko
            if sum(target['nl_thieu'].values()) == 1 and target['nl_thieu']['auto_color'] == 1  \
                ...
                # Úp thẻ, tìm nl trả
                stocks_return = self.Tim_nl_tra(target['the'], n=9)
                # print('3333')
                return [], target['the'], stocks_return, 3
            ...
            for i in range(nn):
                temp_list_mau = [mau for mau in state['Board']._Board__stocks.keys() if mau != 'auto_color' and state['Board']._Board__stocks[mau] != 0 and mau not in stocks]
    ...
    def Tim_nl_tra(self, card, stocks=[], n=10):
    ...
    def the_co_the_lay(self, board, mau_the_quan_trong):
        ...
        for car in list_card_can_check:
            nl_thieu = deepcopy(car._Card__stocks)
            nl_thieu['auto_color'] = 0
            for mau in temp:
                nl_thieu[mau] = max(0, nl_thieu[mau] - self._Player__stocks_const[mau] - self._Player__stocks[mau])
        
            if self._Player__stocks['auto_color'] != 0:
                a = self._Player__stocks['auto_color']
                for i in range(a):
                    if sum(nl_thieu.values()) != 0:
            ...
            if sum(nl_thieu.values()) != 0:
                nl_vc_thieu = deepcopy(car._Card__stocks)
                for mau in temp:
                    nl_vc_thieu[mau] = max(0, nl_vc_thieu[mau] - self._Player__stocks_const[mau])
                
                kha_nang = True
                if sum(nl_vc_thieu.values()) > (10-self._Player__stocks['auto_color']):
                    kha_nang = False
                else:
                    nl_nh_thieu = deepcopy(nl_thieu)
                    for mau in temp:
                        nl_nh_thieu[mau] = max(0, nl_nh_thieu[mau] - board._Board__stocks[mau])
