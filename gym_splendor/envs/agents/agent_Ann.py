...
class Agent(Player):
    ...
    def action(self, state):
        ...
        if list_co_the_lay.__len__() != 0:
            ...
            if mau_target_thieu.__len__() == 1:
                if target['nl_thieu'][mau_target_thieu[0]] >= 2 and state['Board']._Board__stocks[
                    mau_target_thieu[0]] >= 4:
            ...
            nn = 3 - stocks.__len__()
            for i in range(nn):
                temp_list_mau = [mau for mau in state['Board']._Board__stocks.keys() if
    ...
    def the_co_the_lay_func(self, board, mau_the_quan_trong):
        ...
        for car in (list_check_1 + list_check_1_2 + list_check_2):
            ...
            for mau in temp:
                nl_thieu[mau] = max(0, car._Card__stocks[mau] - self._Player__stocks_const[mau] - self._Player__stocks[
                    mau])
