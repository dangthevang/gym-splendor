...
class Agent(Player):
    ...
    def action(self, state):
        ...
        if (card is None) and (len(stocks) == 0):
            if len(self.card_upside_down) < 3:
                card = self.card_target(state['Board'])
    ...
    def card_target(self, board):
        cards = []
        for key in board.dict_Card_Stocks_Show.keys():
            if key != 'Noble':
                cards.extend(board.dict_Card_Stocks_Show[key])
        review_card_by_coin = [sum(self.lack_of_coin(card).values()) for card in cards]
        return cards[review_card_by_coin.index(min(review_card_by_coin))]
