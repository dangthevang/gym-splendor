...
class Agent(Player):
    ...
    def dict_up(self, board):
        ...
        for the in list_co_the_up:
            dict_up['U' + str(the._Card__id)] = self.cham_diem(board, the._Card_Stock__type_stock) / sum(
                the._Card__stocks.values()) + self.cham_diem(board, 'auto_color') * min(1, board._Board__stocks[
                'auto_color'])
            if the == thet2:
                return {'U' + str(the._Card__id): self.cham_diem(board, the._Card_Stock__type_stock) / sum(
                    the._Card__stocks.values()) + self.cham_diem(board, 'auto_color') * min(1, board._Board__stocks[
                    'auto_color'])}
    ...
    def dict_mo(self, board):
        ...
        for the in list_the_can_buy:
            chi_phi = 0
            for nguyenlieu in temp:
                if the._Card__stocks[nguyenlieu] - self._Player__stocks_const[nguyenlieu] > self._Player__stocks[
                    nguyenlieu]:
                    ...
                    chi_phi += (the._Card__stocks[nguyenlieu] - self._Player__stocks_const[
                        nguyenlieu]) * self.cham_diem(board, nguyenlieu)
    ...
    def cham_diem(self, board, nglieu):
        ...
        for nguyenlieu in temp:
            ...
            diem = (the._Card__score / so_nl_thieu - the._Card__score / (so_nl_thieu + 1)) * max(0, (
                        the._Card__stocks[nguyenlieu] - self._Player__stocks_const[nguyenlieu] - self._Player__stocks[
                    nguyenlieu])) / so_nl_thieu
        ...
        for the in list_the:
            if the not in list_the_can_buy:
                ...
                for nguyenlieu in the._Card__stocks.keys():
                    ...
                    if max(0, (the._Card__stocks[nguyenlieu] - self._Player__stocks_const[nguyenlieu] -
                               self._Player__stocks[nguyenlieu])) > so_nl_thieu:
                        so_nl_thieu = max(0, (the._Card__stocks[nguyenlieu] - self._Player__stocks_const[nguyenlieu] -
                                              self._Player__stocks[nguyenlieu]))
